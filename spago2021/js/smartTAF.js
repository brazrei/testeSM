...
function getAMDStatus(TAF) {
    ...
    if (isLinux()) {
        dif = new Date(dif).addHours(3)
        dataZero = new Date(dataZero).addHours(3)
        horasValid = new Date(horasValid).addHours(3)
    }
    
    let prazoFinal = inicio.addHours(horasValid.getHours()/6)
...
function getTAFFromMetar(metar){
    ...
    if (arrayTAFs[loc])
        return {localidade: loc, TAF: arrayTAFs[loc].TAF, visibilidade: getVisPredHora(arrayTAFs[loc].TAF, dh), teto: getTetoHora(arrayTAFs[loc].TAF, dh), inicioValid: getBeginTAF(arrayTAFs[loc].TAF), permiteAMD: getAMDStatus(arrayTAFs[loc].TAF)}
    else
